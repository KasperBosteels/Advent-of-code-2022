...
let used: number = allnodes.filter((n) => n.name == "/")[0].size
//allnodes.map((n) => used += n.size)
const deletablenodes = allnodes.filter((n) => n.size > 100000).sort((a, b) => a.size < b.size ? -1 : 1)
let nodesLargeEnough: n[] = []
const totalDisk = 70000000 //total space
const minspace = 30000000 //min space needed 

console.log("disk size: ", totalDisk)
console.log("min update ", minspace)
console.log("curr used: ", used)
console.log("disk-free: ", totalDisk - used)
...
nodesLargeEnough.map((n) => {
    console.log(n.size, " + ", totalDisk - used, " = ", n.size + totalDisk - used)
})
